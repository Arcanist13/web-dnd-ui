...
async def get_spells():
  '''Get all spells.'''
  spells = get_db_all("SELECT id, name, level, cast_time, damage, attack_save, components, duration FROM spells")
...
async def get_class_spells(class_str: str):
  ...
  spell = get_db_all('''
    ...
    WHERE name IN (SELECT name FROM {class_name}_spells NOCASE) COLLATE NOCASE;
